...
<!--            </a-form-model-item>-->
          ...
          <a-col :span="24">
            <a-form-model-item label="卡号"  :labelCol="labelCol" :wrapperCol="wrapperCol" prop="number">
              <a-input :disabled="model.number !== null" v-model="model.number" placeholder="请输入卡号"  ></a-input>
            </a-form-model-item>
          </a-col>
          <a-col :span="24">
            <a-form-model-item label="支付密码" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="password">
              <a-input-password v-model="model.password" placeholder="请输入支付密码" v-if="!disabled" />
              <a-input v-model="model.password" placeholder="请输入支付密码"  v-else/>
            </a-form-model-item>
          </a-col>
          <a-col :span="24">
            <a-form-model-item label="绑定车辆" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="carId">
              <j-search-select-tag v-model="model.carId" :dict="dictCarQuery"  />
            </a-form-model-item>
          </a-col>
          <a-col :span="24">
            <a-form-model-item label="绑定车位" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="parkingId">
              <j-search-select-tag v-model="model.parkingId" :dict="dictQueryCarParking"  />
            </a-form-model-item>
          </a-col>
        </a-row>
      </a-form-model>
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      // alert(this.localUserInfo.id)
      this.dictCarQuery = "car,number,id, user_id =\'"+this.localUserInfo.id+"\'"
    },
    methods: {
      ...mapGetters(["nickname"]),
      /**
       ...
       */
      getUserInfo(){
        getUserList({nickname:this.nickname()}).then(
          res=>{
      ...
      add () {
        ...
        this.dictQueryCarParking ="car_parking,parking_number,id,type = 2 and status = '1' "
      },
      edit (record) {
        this.model = Object.assign({}, record);
        this.model.olderParkingId = record.parkingId
        console.log(record)
        // 选择临时车位且是非固定
        this.dictQueryCarParking ="car_parking,parking_number,id,type = 2 and status = '1' or id =\'"+record.parkingId+"\'"
        // this.dictQueryCarParking ="car_parking,parking_number,id,type = 2 or id =\'"+record.parkingId+"\'"
        this.visible = true;
      },
      submitForm () {
        const that = this;
